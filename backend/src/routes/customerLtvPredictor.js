...
// Customer lifetime value prediction to guide acquisition spend
// Feature: customer-ltv-predictor
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Customer lifetime value prediction to guide acquisition spend.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'customer-ltv-predictor', payload, result);
    res.json({ success: true, feature: 'customer-ltv-predictor', ...result });
  } catch (err) {
    console.error('customer-ltv-predictor analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Customer lifetime value prediction to guide acquisition spend. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'customer-ltv-predictor', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'customer-ltv-predictor', title: 'Customer lifetime value prediction to guide acquisition spend', project: 'AISubscriptionBoxCurator' });
});
