...
// Logistics-integrated address validation and label generation
// Feature: logistics-address-validator
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AISubscriptionBoxCurator" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Logistics-integrated address validation and label generation.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'logistics-address-validator', payload, result);
    res.json({ success: true, feature: 'logistics-address-validator', ...result });
  } catch (err) {
    console.error('logistics-address-validator analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AISubscriptionBoxCurator". Return strict JSON only.',
      `Feature: Logistics-integrated address validation and label generation. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'logistics-address-validator', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'logistics-address-validator']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'logistics-address-validator', title: 'Logistics-integrated address validation and label generation', project: 'AISubscriptionBoxCurator' });
});
