...
export default function CfCompetitorPriceMonitoringWithRepricingRecommendations() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-competitor-price-monitoring-with-repricing-recommendations/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-competitor-price-monitoring-with-repricing-recommendations', project: 'AISubscriptionBoxCurator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-competitor-price-monitoring-with-repricing-recommendations/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Competitor price monitoring with repricing recommendations</h1>
