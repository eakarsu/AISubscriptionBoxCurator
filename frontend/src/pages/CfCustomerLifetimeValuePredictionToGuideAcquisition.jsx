...
export default function CfCustomerLifetimeValuePredictionToGuideAcquisition() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-customer-lifetime-value-prediction-to-guide-acquisition-spend/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-customer-lifetime-value-prediction-to-guide-acquisition-spend', project: 'AISubscriptionBoxCurator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-customer-lifetime-value-prediction-to-guide-acquisition-spend/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Customer lifetime value prediction to guide acquisition spend</h1>
