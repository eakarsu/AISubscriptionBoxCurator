...
export default function CfMultiArmedBanditPreferenceLearningLoopFor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-armed-bandit-preference-learning-loop-for-product-discovery/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-armed-bandit-preference-learning-loop-for-product-discovery', project: 'AISubscriptionBoxCurator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-armed-bandit-preference-learning-loop-for-product-discovery/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-armed bandit preference learning loop for product discovery</h1>
