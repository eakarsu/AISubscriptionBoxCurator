...
export default function CfSeasonalDemandForecastingForInventorySpikesAround() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-seasonal-demand-forecasting-for-inventory-spikes-around-holidays/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-seasonal-demand-forecasting-for-inventory-spikes-around-holidays', project: 'AISubscriptionBoxCurator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-seasonal-demand-forecasting-for-inventory-spikes-around-holidays/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Seasonal demand forecasting for inventory spikes around holidays</h1>
