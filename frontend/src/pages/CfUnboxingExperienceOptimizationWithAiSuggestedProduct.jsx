...
export default function CfUnboxingExperienceOptimizationWithAiSuggestedProduct() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-unboxing-experience-optimization-with-ai-suggested-product-arrangement/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-unboxing-experience-optimization-with-ai-suggested-product-arrangement', project: 'AISubscriptionBoxCurator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-unboxing-experience-optimization-with-ai-suggested-product-arrangement/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Unboxing experience optimization with AI-suggested product arrangement</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AISubscriptionBoxCurator</p>
      </div>
