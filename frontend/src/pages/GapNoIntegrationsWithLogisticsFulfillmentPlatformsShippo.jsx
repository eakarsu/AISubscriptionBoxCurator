...
export default function GapNoIntegrationsWithLogisticsFulfillmentPlatformsShippo() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-integrations-with-logistics-fulfillment-platforms-shippo-shipstation/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-integrations-with-logistics-fulfillment-platforms-shippo-shipstation', project: 'AISubscriptionBoxCurator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-integrations-with-logistics-fulfillment-platforms-shippo-shipstation/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No integrations with logistics/fulfillment platforms (Shippo, ShipStation)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AISubscriptionBoxCurator</p>
      </div>
